...
public class HungarianAlgorithm {
	...
	//*****************************//
	// CALCULATE 2ND BEST SOLUTION //
	//*****************************//
	public static int[] get2ndSolution() {
		double[][] cost2 = copyOf(cost1);
		int nbRows = cost2.length;
		int rowLength = cost2[0].length;
		double[] solution = new double[nbRows];
		int minCost = Integer.MAX_VALUE;
		double[] cursol = new double[nbRows];
		cursol = nextSol(cursol,rowLength,nbRows);
		while(hasNextSol(cursol,rowLength,nbRows)) {
			int cost = getCost(cursol,cost2);
			if(cost > 0 && cost<minCost) {
				minCost = cost;
				System.arraycopy(cursol, 0, solution, 0, cursol.length);
			}
			cursol = nextSol(cursol,rowLength,nbRows); 
		}
		int[] intsolution = new int[nbRows];
	...
	public static int getCost(double[] cursol, double[][] cost) {
		int curCost = 0;
		for(int i=0; i<cursol.length; i++) {
			curCost+=cost[i][(int) cursol[i]];
		}
		return curCost;
	}
	
	public static double[] nextSol(double[] cursol, int rowLength, int nbRows) {
		int curRow = nbRows-1;
		
		if(cursol[curRow] == rowLength-1) {
			while(curRow>=0) {
				if(cursol[curRow] == rowLength-1) {
					cursol[curRow] = 0;
					curRow--;
		...
		if(hasDuplicates(cursol)) {
			return nextSol(cursol,rowLength,nbRows);
		} else {
			return cursol;
		}
	}
	
	public static boolean hasNextSol(double[] cursol, int rowLength, int nbRows) {
		// maak last solution
		double[] lastSol = new double[rowLength];
		for(int i=0; i<rowLength; i++) {
			lastSol[i] = rowLength - i -1;
		}
		return !Arrays.equals(lastSol, cursol);
	}
	
	public boolean solContains(double[] sol, double i) {
		boolean b = false;
	...
	public static boolean hasDuplicates(double[] cursol) {
...
//		printTime((endTime - startTime)/1000000000.0);
		...
		int[] pipi = get2ndSolution();
		
		System.out.println("");
		for(int i=0; i<pipi.length; i++) {
			System.out.print(pipi[i]+" ");
		}
	}
}
